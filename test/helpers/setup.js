...
const createMockTaskModel = (Op, taskData, taskIdCounter) => {
  return {
    create: async data => {
      const task = {
...
const createFastify = async (options = {}, taskData, taskIdCounter) => {
  const sinon = require('sinon');
  ...
  const mockTaskModel = createMockTaskModel(Op, taskData, taskIdCounter);
  ...
  const taskOptions = {
    ...
    pollInterval: 100,
    task: {
      'test-type': async ({ task, result }) => result,
      'polling-type': async ({ task, result }) => result,
      'next-type': async ({ task, result, context }) => result,
      'progress-type': async ({ task, result }) => result,
      'fail-type': async ({ task, result }) => result,
      'hang-type': async ({ task, result }) => result,
      'log-type': async ({ task, result }) => result,
      'polling-fail-type': async ({ task, result }) => result,
      'polling-pending-type': async ({ task, result }) => result
    },
